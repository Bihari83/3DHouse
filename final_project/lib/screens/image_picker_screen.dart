...
//     final uri = Uri.parse(
//         "https://32f9-112-134-33-122.ngrok-free.app/upload");
//     final request = http.MultipartRequest("POST", uri);
//     request.files.add(await http.MultipartFile.fromPath('image', _image!.path));
...
class _ImagePickerScreenState extends State<ImagePickerScreen> {
  File? _image;
  bool _uploading = false; // Added _uploading variable
  final picker = ImagePicker();
  ...
  Future<void> uploadImage() async {
    ...
    // Show loading spinner for the upload button
    setState(() {
      _uploading = true;
    });

    final uri = Uri.parse(
        "https://32f9-112-134-33-122.ngrok-free.app/upload"); // Replace with your server URL
    final request = http.MultipartRequest("POST", uri);
    request.files.add(await http.MultipartFile.fromPath('image', _image!.path));
    ...
    try {
      ...
      if (response.statusCode == 200) {
        // Handle success response
        print('Image uploaded successfully.');
        // Navigate back to the same page after successful upload
      ...
      // Hide loading spinner for the upload button
      setState(() {
        _uploading = false;
      });
    }
          ...
          children: [
            ...
            if (_image != null) // Conditionally render the upload button
              Row(
                ...
                children: [
                  _uploading
                      ? const CircularProgressIndicator() // Show loading spinner for the upload button
                      : ElevatedButton(
                          onPressed: uploadImage,
                          child: const Text('Upload'),
                        ),
